...
func tableOffice365MailMyMessage(_ context.Context) *plugin.Table {
	return &plugin.Table{
		Name:        "office365_mail_my_message",
		Description: "Retrieves messages in the specified user's mailbox.",
		List: &plugin.ListConfig{
			Hydrate: listOffice365MailMyMessages,
...
func listOffice365MailMyMessages(ctx context.Context, d *plugin.QueryData, h *plugin.HydrateData) (interface{}, error) {
	...
	if err != nil {
		logger.Error("listOffice365MailMyMessages", "create_iterator_instance_error", err)
		return nil, err
	}
	...
	if err != nil {
		logger.Error("listOffice365MailMyMessages", "paging_error", err)
		return nil, err
	}
